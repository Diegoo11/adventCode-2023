...
function res(cadena) {
  ...
  const arr2 = arr.map((str) => {
    const blue = [];
    let draft = str;
    while (draft.indexOf('blue') !== -1) {
      blue.push(Number(draft.substring(draft.indexOf('blue') - 3, draft.indexOf('blue'))));
      draft = draft.substring(draft.indexOf('blue') + 4);
    }
    const red = [];
    draft = str;
    while (draft.indexOf('red') !== -1) {
      red.push(Number(draft.substring(draft.indexOf('red') - 3, draft.indexOf('red'))));
      draft = draft.substring(draft.indexOf('red') + 4);
    }

    const green = [];
    draft = str;
    while (draft.indexOf('green') !== -1) {
      green.push(Number(draft.substring(draft.indexOf('green') - 3, draft.indexOf('green'))));
      draft = draft.substring(draft.indexOf('green') + 4);
    }
  ...
  arr2.forEach((obj, index) => {
    if (
      obj.blue.every((blue) => blue <= 14)
      && obj.red.every((red) => red <= 12)
      && obj.green.every((green) => green <= 13)
    ) {
      suma += index + 1;
    }
  });
...
function res(cadena) {
  ...
  arr2.forEach((obj) => {
    const blue = Math.max(...obj.blue);
    const red = Math.max(...obj.red);
    const green = Math.max(...obj.green);
