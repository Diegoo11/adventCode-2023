...
const input = `...............776..............552........968..................589...26...........484..............958......186....546.........484.........
.........*.........778....................*....124...................................*...............*........%..26.........................
...
function search(str, x) {
  if (!isNumber(str[x])) return 1;
  let rpta = '';
  while (true) {
    if (isNumber(str[x - 1])) x -= 1;
    else break;
  }
  while (true) {
    if (isNumber(str[x])) {
  ...
  arr2.forEach((str, index) => {
    ...
    for (let i = 0; i < str.length; i += 1) {
      if (isNumber(str[i])) {
        ...
        finalNum = i - 1;
        x : for (let j = -1; j < 2; j += 1) {
          if (!arr[index + j]) continue;
          for (let x = initNum - 1; x <= finalNum + 1; x += 1) {
            if (!arr[index + j][x]) continue;
            if (!isNumber(arr[index + j][x]) && arr[index + j][x] !== '.') {
              suma += Number(num);
              break x;
            }
          }
...
function res(cadena) {
  ...
  arr.forEach((str, yIndex) => {
    str.split('').forEach((char, xIndex) => {
      if (char === '*') {
        let nums = [];
        let multiplicacion = 1;
        for (let y = -1; y < 2; y += 1) {
          if (!arr[yIndex + y]) continue;
          for (let x = -1; x < 2; x += 1) {
            if (x === 0 && y === 0) continue;
            if (isNumber(arr[yIndex + y][xIndex + x])) {
              nums.push(Number(search(arr[yIndex + y], xIndex + x)));
            }
          }
        }
        nums = [...(new Set(nums))];
        if (nums.length > 1) {
          console.log(nums);
          nums.forEach((n) => { multiplicacion *= n; });
