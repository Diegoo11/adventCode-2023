function res(input) {
  ...
  const X = map[Y].findIndex((item) => item === 'S');
  ...
  let count = 0;
  while (init.lether !== 'S') {
    nido.push([init.y, init.x]);
    const after = list[init.lether][0] === init.before
      ? list[init.lether][1] : list[init.lether][0];
    init.before = invert[after];
    const [y, x] = direction[after];
    init.x += x;
    init.y += y;
    init.lether = map[init.y][init.x];
    count += 1;
  }

  const onlyNido = map.map((line, y) => line.map((item, x) => {
    if (item === '.') return item;
    if (nido.findIndex(([ny, nx]) => ny === y && nx === x) !== -1) return item;
    return '*';
  }).join(''));
  console.log(onlyNido);
  return (count - 1) / 2 + 1;
}
